...
addevent.onclick=function(){
  var addcontent = addinput.value.replace( /^\s/, '');
  if(addcontent==""){
    alert("尚未輸入項目");
  }else{
    additem(addcontent);
...
function addDoneItem(doneContent){
  ...
  doneItemIconsDone.onclick=function(){
    additem(doneItemText.innerHTML);
    doneItem.parentNode.removeChild(doneItem);
  }
}
...
//   modifyBtn[i].index=i;
//   modifyBtn[i].onclick=function(ev){
//     ev=window.event || ev;
//     var activeWaitItemText=waitItemText[this.index];
//     var waitItemBefore=activeWaitItemText.innerHTML;
//     activeWaitItemText.innerHTML="<input class="+'"wait__item__text__input"'+'type="text">';
//     var waitItemTextInput = document.getElementsByClassName('wait__item__text__input')[0];
//     waitItemTextInput.value= waitItemBefore;
//     waitItemTextInput.focus();
//     ev.stopPropagation ? ev.stopPropagation() : ev.cancelBubble=true;
//     // modifyBtn[0].stopPropagation ? modifyBtn[0].stopPropagation() : modifyBtn[0].cancelBubble=true;
//     console.log(ev);
//     document.onclick=function(){
//       activeWaitItemText.innerHTML=waitItemTextInput.value;
//     }
//   }
// }
